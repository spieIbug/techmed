...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/acte-medical'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ActeMedical) {
        return item.id;
    }
    registerChangeInActeMedicals() {
        this.eventSubscriber = this.eventManager.subscribe('acteMedicalListModification', (response) => this.loadAll());
    }
